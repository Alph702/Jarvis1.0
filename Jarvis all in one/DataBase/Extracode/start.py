...
def GoogleImage():
    oo = open("F:\\Code\\Jarvis all in one\\Data.txt",'rt')
    query = str(oo.read())
    oo.close
    pppp = open("F:\\Code\\Jarvis all in one\\Data.txt",'r+')
    pppp.truncate(0)
    pppp.close()

    webdriver = "F:\\Code\\Jarvis all in one\\DataBase\\webdriver\\chromedriver.exe"
    photos = "F:\\Code\\Jarvis all in one\\DataBase\\GooglePhotos\\"
